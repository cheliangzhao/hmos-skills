...
| 成员属性名禁用内置方法名 | @Component/@ComponentV2 struct 成员属性名**禁用** ArkUI 内置链式方法名（`id`/`width`/`height`/`margin`/`padding`/`offset`/`position` 等），与 `CustomComponent` 基类方法签名冲突 → **10505001**；改用 `xxxId`/`xxxWidth` 等业务命名 |
