...
| virtualScroll 模式需设置 cachedCount | cachedCount 设在**滚动容器**（List/Grid/WaterFlow）上，**不在 Repeat 链**（Repeat 链无 cachedCount，链上报 10505001） |
| virtualScroll 需传 totalCount | `virtualScroll({ totalCount: arr.length })`，否则虚拟滚动总数未知 |
| key() 必须唯一 | — |
