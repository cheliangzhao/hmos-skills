...
**动作**：调用 `hmos-arkui-knowledge-retriever` 检索本地知识库查询官方文档（与 SKILL.md Step 2.3 一致，不使用 MCP）。

```bash
python {hmos-arkui-knowledge-retriever}/scripts/run.py query "组件名 具体问题" --top-k 5 --include-code --format json
→ 返回准确的官方 API 签名和示例代码
```
