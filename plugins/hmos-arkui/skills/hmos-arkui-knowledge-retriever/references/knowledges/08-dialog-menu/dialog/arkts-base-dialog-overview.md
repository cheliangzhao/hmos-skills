...
| 名称 | 描述 |
| --- | --- |
|[不依赖UI组件的全局自定义弹出框 (openCustomDialog)](arkts-open-custom-dialog.md) | 当用户需要在自定义弹出框内动态更新弹出框属性时使用。 |
|[基础自定义弹出框 (CustomDialog)](arkts-common-components-custom-dialog.md) | 当用户需要自定义弹出框内的组件和内容时使用。 |
| [固定样式弹出框](arkts-fixed-style-dialogs.md) | 固定布局格式，只需输入文本内容和按钮操作，包含AlertDialog、ActionSheet、PickerDialog等。 |
| [警告弹窗 (AlertDialog)](ts-methods-alert-dialog-box.md) | 固定样式，通常用来展示用户当前需要或必须关注的信息或操作。如用户操作一个敏感行为时响应一个二次确认的弹出框。 |
| [列表选择弹窗 (ActionSheet)](ts-methods-action-sheet.md) | 固定样式，当用户需要关注或确认的信息存在列表选择时使用。 |
|[选择器弹窗 (PickerDialog)](arkts-fixed-style-dialogs.md#datepickerdialog-日期选择器) | 固定样式，当用户需要在弹出框内选择日期、时间和文本时使用。 |
...
| [弹出框层级管理](arkts-dialog-level-management.md) | 从API version 18开始，可以通过设置[levelOrder](../reference/apis-arkui/js-apis-promptAction.md#basedialogoptions11)参数来管理弹出框的显示顺序。 |
...
| [弹出框焦点策略](arkts-dialog-focus-strategy.md) | 从API version 19开始，可以通过设置[focusable](../reference/apis-arkui/js-apis-promptAction.md#basedialogoptions11)参数来管理弹出框是否获取焦点。 |
| [弹出框蒙层控制](arkts-dialog-mask-control.md) | 开发者可以通过设置maskColor、maskRect等参数来对弹出框蒙层进行定制。 |
