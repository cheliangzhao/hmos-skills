...
| start | number | 否 | 开始填充的索引，如果`start < 0`，则会从`start + Int32Array.length`位置开始。默认值为0。|
| end | number | 否 | 结束填充的索引（不包括该元素），如果`end < 0`，则会到`end + Int32Array.length`位置结束。默认为ArkTS Int32Array的长度。|
...
| 参数名 | 类型   | 必填 | 说明                                                |
| ------ | ------ | ---- | ------------------------------------------------- |
| begin  | number | 否   | 开始索引，如果`begin < 0`，则会从`begin + Int32Array.length`位置开始。默认值为0。 |
| end    | number | 否   | 结束索引（不包括该元素），如果`end < 0`，则会到`end + Int32Array.length`位置结束。默认为ArkTS Int32Array的长度。 |
...
| fromIndex  | number | 否  | 开始搜索的索引，如果`fromIndex < 0`，则会从`fromIndex + Int32Array.length`位置开始。默认值为0。|
